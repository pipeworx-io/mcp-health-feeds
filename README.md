...
`tools/list` at `https://gateway.pipeworx.io/health-feeds/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/health_feeds_list_feeds \
  -H 'Content-Type: application/json' \
  -d '{"category":"health"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/health_feeds_list_feeds`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "health-feeds": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-health-feeds"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-health-feeds
```
